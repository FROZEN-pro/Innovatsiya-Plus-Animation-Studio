...
export default function SubscriptionModal() {
  const { isSubscriptionModalOpen, setSubscriptionModalOpen, language } = useAppStore();
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 backdrop-blur-md">
      <motion.div 
        ...
        className="relative w-full max-w-2xl bg-[#0d0d12] border border-white/10 rounded-[32px] p-6 sm:p-8 shadow-2xl text-white overflow-hidden"
      >
        ...
        <div className="flex items-center gap-3 mb-6">
          ...
          <div>
            ...
            <p className="text-xs text-white/60">Automated Monthly Ad-Free High-Definition Access</p>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-6">
          ...
          >
            <div className="flex justify-between items-start mb-3">
              <div>
                <h3 className="font-bold text-sm text-white">Pro Subscriber</h3>
                <p className="text-[10px] text-white/50">Full HD Streaming</p>
              </div>
              <span className="text-lg font-black text-orange-400">$9.99<span className="text-[10px] text-white/40 font-normal">/mo</span></span>
            </div>
            <ul className="space-y-1.5 text-[11px] text-white/70">
              <li className="flex items-center gap-1.5"><Check size={12} className="text-orange-400" /> Ad-free 1080p 60fps</li>
              <li className="flex items-center gap-1.5"><Check size={12} className="text-orange-400" /> 10 encrypted offline downloads</li>
              <li className="flex items-center gap-1.5"><Check size={12} className="text-orange-400" /> All video & music categories</li>
            </ul>
          </div>
          ...
          >
            ...
            <div className="flex justify-between items-start mb-3">
              <div>
                <h3 className="font-bold text-sm text-white flex items-center gap-1">VIP Creator <Sparkles size={12} className="text-amber-400" /></h3>
                <p className="text-[10px] text-white/50">4K Ultra HD + Spatial Audio</p>
              </div>
              <span className="text-lg font-black text-amber-400">$19.99<span className="text-[10px] text-white/40 font-normal">/mo</span></span>
            </div>
            <ul className="space-y-1.5 text-[11px] text-white/80">
              <li className="flex items-center gap-1.5"><Check size={12} className="text-amber-400" /> Unlimited 4K HDR & Spatial Music</li>
              <li className="flex items-center gap-1.5"><Check size={12} className="text-amber-400" /> Unlimited AES-256 Offline Downloads</li>
              <li className="flex items-center gap-1.5"><Check size={12} className="text-amber-400" /> Gemini AI Script & Subtitle Tools</li>
            </ul>
          </div>
        ...
        <div className="mb-6 space-y-2">
          <label className="text-[10px] font-bold uppercase tracking-wider text-white/50">Automated Billing Method</label>
          <div className="grid grid-cols-3 gap-2">
            ...
            <button
              ...
              className={`flex items-center justify-center gap-2 py-2.5 px-3 rounded-xl border text-xs font-medium transition-all ${
                paymentMethod === 'card' ? 'bg-white/10 border-orange-500 text-white' : 'bg-white/5 border-white/10 text-white/60'
              }`}
            >
            ...
            <button
              ...
              className={`flex items-center justify-center gap-2 py-2.5 px-3 rounded-xl border text-xs font-medium transition-all ${
                paymentMethod === 'gpay' ? 'bg-white/10 border-orange-500 text-white' : 'bg-white/5 border-white/10 text-white/60'
              }`}
            >
            ...
            <button
              ...
              className={`flex items-center justify-center gap-2 py-2.5 px-3 rounded-xl border text-xs font-medium transition-all ${
                paymentMethod === 'telegram' ? 'bg-white/10 border-orange-500 text-white' : 'bg-white/5 border-white/10 text-white/60'
              }`}
            >
